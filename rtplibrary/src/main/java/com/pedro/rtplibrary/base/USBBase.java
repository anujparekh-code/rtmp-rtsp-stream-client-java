...
//        audioTrack = -1;
    ...
    private void prepareGlView() {
        if (glInterface != null) {
            if (glInterface instanceof OffScreenGlThread && Build.VERSION.SDK_INT >= 18) {
                glInterface = new OffScreenGlThread(context);
...
//        videoEncoder.stopSendBlackImage();
    ...
    public void getAacData(ByteBuffer aacBuffer, MediaCodec.BufferInfo info) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2 && recordController.isRecording() && canRecord) {
